...
`KBEngine` 的底层时间抽象在 `kbe/src/lib/common/timestamp.h:64`：
...
- `kbe/src/lib/network/event_dispatcher.cpp:85`
- `kbe/src/lib/network/event_dispatcher.cpp:151`
- `kbe/src/lib/network/event_dispatcher.cpp:177`
...
- 定义在 `kbe/src/lib/server/serverapp.cpp:34`
- `ServerApp::time()` 返回它，`kbe/src/lib/server/serverapp.h:84`
- `gameTimeInSeconds()` 只是 `g_kbetime / gameUpdateHertz`，`kbe/src/lib/server/serverapp.cpp:239`
...
实体型进程的推进点在 `kbe/src/lib/server/entity_app.h:698`：
...
`KBEngine` 的脚本定时器在 `kbe/src/lib/server/script_timers.cpp:40`：
...
- `kbe/src/server/baseapp/baseapp.cpp:430`
- `kbe/src/server/baseapp/baseapp.cpp:504`
- `kbe/src/server/cellapp/cellapp.cpp:172`
- `kbe/src/server/cellapp/cellapp.cpp:225`
...
- `kbe/src/server/dbmgr/sync_app_datas_handler.cpp:126`
- `kbe/src/lib/server/entity_app.h:1230`
- `kbe/src/lib/server/entity_app.h:1247`
...
- 定义与访问在 `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/server_app.hpp`
- 推进逻辑在 `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/server_app.cpp:311`
...
- `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/entity_app.hpp`
- `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/entity_app.cpp`
...
- 类型定义在 `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.hpp:13`
- 构造和核心逻辑在 `BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:24`
...
- `readingNow()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:240`
  - 不是直接返回整数 tick
  - 而是结合“下一次 tick 送达时间”和当前 `timestamp()` 计算一个连续读数

- `synchroniseWithMaster()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:283`
  - 发请求向 master 询问当前世界时间读数

- `inputMasterReading()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:110`
...
- `CellAppMgr` 创建的是 master `TimeKeeper`
  - `BigWorld-Engine-14.4.1/programming/bigworld/server/cellappmgr/cellappmgr.cpp:1781`

- `BaseApp`、`CellApp`、`BaseAppMgr` 创建的是指向 `CellAppMgr` 的 slave `TimeKeeper`
  - `BigWorld-Engine-14.4.1/programming/bigworld/server/baseapp/baseapp.cpp:2049`
  - `BigWorld-Engine-14.4.1/programming/bigworld/server/cellapp/cellapp.cpp:816`
  - `BigWorld-Engine-14.4.1/programming/bigworld/server/baseappmgr/baseappmgr.cpp:2469`
...
- `BigWorld-Engine-14.4.1/programming/bigworld/server/baseapp/baseapp.cpp:759`
- `BigWorld-Engine-14.4.1/programming/bigworld/server/cellapp/cellapp.cpp:674`
- `BigWorld-Engine-14.4.1/programming/bigworld/server/cellapp/cellapp.cpp:1603`
- `BigWorld-Engine-14.4.1/programming/bigworld/server/cellappmgr/cellapps.cpp:540`
...
- `CellAppMgr::writeGameTimeToDB()`：`BigWorld-Engine-14.4.1/programming/bigworld/server/cellappmgr/cellappmgr.cpp:2492`
- `DBApp::writeGameTime()`：`BigWorld-Engine-14.4.1/programming/bigworld/server/dbapp/dbapp.cpp:2732`
...
- `timestamp_gettimeofday()`：`kbe/src/lib/common/timestamp.h:52`
- `timestamp_gettime()`，底层是 `CLOCK_MONOTONIC`：`kbe/src/lib/common/timestamp.h:64`
- `timestamp_rdtsc()`：`kbe/src/lib/common/timestamp.h:34`

当前这个分支还有一个很重要的实现细节：

- `kbe/src/lib/common/timestamp.cpp:8` 定义了 `KBE_USE_RDTSC`
- `kbe/src/lib/common/timestamp.cpp:11` 把 `g_timingMethod` 直接初始化成 `RDTSC_TIMING_METHOD`

这意味着在当前 fork 上，Linux 下默认主链路很可能更偏向 `RDTSC`，而不是 `gettimeofday()`。

而 `EventDispatcher` 的 timer 调度直接依赖 `timestamp()`：

- 注册 timer 时用 `timestamp() + interval`：`kbe/src/lib/network/event_dispatcher.cpp:98`
- 处理 timer 时按 `timestamp()` 判断到期：`kbe/src/lib/network/event_dispatcher.cpp:154`
...
- 全局时间变量定义在 `kbe/src/lib/server/serverapp.cpp:34`
- 每次 game tick 自增一次：`kbe/src/lib/server/entity_app.h:703`
- 脚本 timer 按 `g_kbetime` 处理：`kbe/src/lib/server/serverapp.cpp:253`
...
- `kbe/src/lib/common/timestamp.cpp:102`

但要注意当前这个 fork 的实现细节：

- `kbe/src/lib/common/timestamp.cpp:8` 在 `.cpp` 内直接定义了 `KBE_USE_RDTSC`
- `kbe/src/lib/common/timestamp.cpp:90` 在这个分支下会直接返回 `calcStampsPerSecond_rdtsc()`
...
- `timestamp_gettimeofday()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/cstdmf/timestamp.hpp:74`
- `timestamp_gettime()`，底层 `CLOCK_MONOTONIC`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/cstdmf/timestamp.hpp:82`
- `timestamp_rdtsc()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/cstdmf/timestamp.hpp:56`

Linux 下默认常量是 `GET_TIME_TIMING_METHOD`：

- `BigWorld-Engine-14.4.1/programming/bigworld/lib/cstdmf/timestamp.cpp:35`

也就是默认更偏向 `clock_gettime(CLOCK_MONOTONIC)`，并且 `bwmachined` 会把 timing method 传给子进程：

- `BigWorld-Engine-14.4.1/programming/bigworld/server/tools/bwmachined/linux_machine_guard.cpp:413`

`BigWorld` 的主逻辑 tick 也是通过 `timestamp()` 驱动推进：

- `advanceTime()` 里用 `timestamp() - lastAdvanceTime_` 计算 tick period：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/server_app.cpp:313`
- 然后 `++time_`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/server_app.cpp:324`

而 `TimeKeeper` 也是基于 `timestamp()` 去估算当前世界时钟读数：

- 接收 master 读数时记录 `timestamp()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:114`
- `readingNow()` 里按当前 `timestamp()` 估算连续读数：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:240`
- 周期同步 master：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/time_keeper.cpp:283`
...
- `DBMgr` 初始化完成后发送 `gametime`：`kbe/src/server/dbmgr/sync_app_datas_handler.cpp:126`
- `EntityApp` 收到后直接设置 `g_kbetime`：`kbe/src/lib/server/entity_app.h:1247`
...
- `ServerApp` 有 `setStartTime()`：`BigWorld-Engine-14.4.1/programming/bigworld/lib/server/server_app.hpp:101`
- `CellApp` 启动时可显式 `setGameTime()`：`BigWorld-Engine-14.4.1/programming/bigworld/server/cellapp/cellapp.cpp:1603`
- `BaseApp` / `CellApp` / `CellAppMgr` 都挂了 `TimeKeeper`
- `CellAppMgr` 会 `writeGameTimeToDB()` 持久化：`BigWorld-Engine-14.4.1/programming/bigworld/server/cellappmgr/cellappmgr.cpp:2492`
- `DBApp` 最终写入数据库：`BigWorld-Engine-14.4.1/programming/bigworld/server/dbapp/dbapp.cpp:2732`
...
- 脚本 timer 注册时直接把当前 `time()` 换算成绝对 tick：`kbe/src/lib/server/script_timers.cpp:54`
- 脚本 timer 每帧按 `g_kbetime` 处理：`kbe/src/lib/server/serverapp.cpp:253`
- 实体位置/朝向变化时间也记录的是 `g_kbetime`：`kbe/src/server/cellapp/entity.cpp:2012`、`kbe/src/server/cellapp/entity.cpp:2059`
