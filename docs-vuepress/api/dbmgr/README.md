...
# KBEngine模块
KBEngine模块[KBEngine](KBEngine.html)KBEngine模块提供了Python脚本控制dbmgr进程处理实体登陆查询与数据存取等部分。
## 成员函数
...
def[addTimer](#addTimer)addTimer( initialOffset, repeatOffset=0, callbackObj=None ):def[delTimer](#delTimer)delTimer( id ):def[executeRawDatabaseCommand](#executeRawDatabaseCommand)executeRawDatabaseCommand( command, callback, threadID, dbInterfaceName ):def[urlopen](#urlopen)urlopen( url, callback, postData, headers ):
## 回调函数
[回调函数](#)回调函数| def onDBMgrReady( ): |
| --- |
| def onDBMgrShutDown( ): |
| def onReadyForShutDown( ): |
| def onSelectAccountDBInterface( accountName ): |

def[onDBMgrReady](#onDBMgrReady)onDBMgrReady( ):def[onDBMgrShutDown](#onDBMgrShutDown)onDBMgrShutDown( ):def[onReadyForShutDown](#onReadyForShutDown)onReadyForShutDown( ):def[onSelectAccountDBInterface](#onSelectAccountDBInterface)onSelectAccountDBInterface( accountName ):
---
...
## 回调函数文档
回调函数文档def onDBMgrReady(  ):

def onDBMgrReady():功能说明：当前进程已经准备好的时候回调此函数。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。def onDBMgrShutDown(  ):

def onDBMgrShutDown():功能说明：进程关闭会回调此函数。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。def onReadyForShutDown(  ):
...
bool，如果返回True，则允许进入进程退出流程，返回其它值则进程会过一段时间后再次询问。def onSelectAccountDBInterface( accountName ):

def onSelectAccountDBInterface(accountName):功能说明：这个回调实现返回某个账号对应的数据库接口，选定接口后dbmgr针对这个账号的相关操作都由对应的数据库接口完成。数据库接口在[kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->dbmgr->databaseInterfaces定义。利用该接口可以根据accountName来决定账号应该存储在哪个数据库。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。参数：

accountName
string，账号的名称。

参数：| accountName | string，账号的名称。 |

accountNamestring，账号的名称。返回：


string，数据库接口名（数据库接口在kbengine_defaults.xml->dbmgr->databaseInterfaces定义）。

返回：| string，数据库接口名（数据库接口在kbengine_defaults.xml->dbmgr->databaseInterfaces定义）。 |

string，数据库接口名（数据库接口在[kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->dbmgr->databaseInterfaces定义）。
---
版权归KBEngine所有。
