...
# KBEngine模块
KBEngine模块[KBEngine](KBEngine.html)KBEngine模块提供了Python脚本控制对特定类型的日志进行分析和转储等能力。
## 成员函数
...
## 回调函数
[回调函数](#)回调函数| def onLoggerAppReady( ): |
| --- |
| def onLoggerAppShutDown( ): |
| def onLogWrote( datas ): |
| def onReadyForShutDown( ): |

def[onLoggerAppReady](#onLoggerAppReady)onLoggerAppReady( ):def[onLoggerAppShutDown](#onLoggerAppShutDown)onLoggerAppShutDown( ):def[onLogWrote](#onLogWrote)onLogWrote( datas ):def[onReadyForShutDown](#onReadyForShutDown)onReadyForShutDown( ):
---
...
## 回调函数文档
回调函数文档def onLoggerAppReady(  ):

def onLoggerAppReady():功能说明：当前进程已经准备好的时候回调此函数。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。def onLoggerAppShutDown(  ):

def onLoggerAppShutDown():功能说明：进程关闭会回调此函数。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。def onLogWrote( datas ):

def onLogWrote(datas):功能说明：如果这个函数在脚本中有实现，logger进程获得了一条新日志，该回调函数被调用。数据库接口在[kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->dbmgr->databaseInterfaces定义。注意：该回调接口必须实现在入口模块([kbengine_defaults.xml](../../keywords.html#kbengine_defaults.xml)kbengine_defaults.xml->entryScriptFile)中。参数：
...
datasbytes，日志数据。def onReadyForShutDown(  ):
