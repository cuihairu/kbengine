...
- 读取出的 HTML 再复用 [tools/rebuild_docs.py](/Users/cui/Workspaces/kbengine/tools/rebuild_docs.py) 中的 `HtmlConverter` 重新生成 Markdown。
- 生成结果逐页对比当前 `docs-vuepress/api/**`。
- `guide/webconsole.md` 使用 [tools/rebuild_docs.py](/Users/cui/Workspaces/kbengine/tools/rebuild_docs.py) 中的 `build_pdf_markdown()` 重新从 PDF 生成后，与当前页面逐字对比。
